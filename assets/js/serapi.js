 ...
 * 2. Call PexelsLoader.setApiKey('...') BEFORE this script auto-inits,
 *    OR set it via a build-time env injection. DO NOT hardcode it in a file
 *    that gets committed to a public repo (see note at bottom of file).
 ...
 * ⚠️ SECURITY NOTE
 * ----------------------------------------------------------------------------
 * This file is loaded client-side. ANY value assigned to PEXELS_API_KEY here
 * is visible to every visitor via "view source" / devtools, regardless of
 * whether the repo is public or private, and regardless of obfuscation.
 * If this file is served from GitHub Pages (or any static host), the key
 * WILL be exposed. To actually hide it, proxy Pexels requests through a
 * server you control (Cloudflare Worker / Netlify Function / Vercel Edge
 * Function) and call THAT endpoint from this script instead of api.pexels.com
 * directly. Ask Claude for a ready-made proxy if you want one.
 * ============================================================================
 */

const PexelsLoader = (() => {
  // ── CONFIG ─────────────────────────────────────────────────────────────
  // Set this via PexelsLoader.setApiKey('...') instead of hardcoding it here.
  let PEXELS_API_KEY = '7f83c49c4ab7a773e871e42237fd4775f124a8abb77e148899d0bbad6d307d69';
  ...
  async function fetchPexels(base, endpoint, params = {}) {
    if (!PEXELS_API_KEY || typeof PEXELS_API_KEY !== 'string') {
      throw new Error(
        `PEXELS_API_KEY is missing or invalid (got: ${JSON.stringify(PEXELS_API_KEY)}). ` +
        `Call PexelsLoader.setApiKey('your-key') before the loader scans the page.`
      );
    }
    if (!base || typeof base !== 'string' || !/^https?:\/\//.test(base)) {
      throw new Error(
        `Pexels base URL is invalid (got: ${JSON.stringify(base)}). ` +
        `Check that PHOTO_BASE / VIDEO_BASE weren't accidentally edited/blanked.`
      );
    }
